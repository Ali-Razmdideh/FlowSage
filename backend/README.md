...
`/scheduled-simulations` turns the same walkthrough into a recurring job: a config pins a persona/flow/goal to a `daily`, `weekly`, or `on_push` interval, and `worker.py`'s cron job (`fire_due_scheduled_simulations`) fires it once it's due, reusing the one-shot `run_simulation_job` path under the hood. Screenshots for the next run are staged ahead of time via `POST /scheduled-simulations/{id}/screenshots` (same filename-ordering convention as `POST /simulations`), and `on_push` configs only fire once a fresh set has been pushed. Each config exposes its friction-score history at `GET /scheduled-simulations/{id}/trend`, one point per completed run, so a flow's regression/improvement over time is visible without re-diffing individual reports.
