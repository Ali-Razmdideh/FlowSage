...
async def start_retraining(
    ...
    try:
        job = await create_retraining_job(session, payload.persona_id)
...
async def stream_retraining(job_id: uuid.UUID, request: Request) -> StreamingResponse:
    session_factory = request.app.state.session_factory
    return StreamingResponse(
        stream_retraining_events(session_factory, job_id), media_type="text/event-stream"
    )
